...
class Bullet {
    constructor(x, y) {
        ...
        this.speed = 12; // Velocidad de la bala
    }
...
class Enemy {
    constructor() {
        ...
        this.speed = 1.5; // Velocidad del enemigo
...
function drawlives(text) {
    ctx.font = '20px Arial';           // Establece la fuente y tamaño del texto
    ctx.fillStyle = 'white';            // Establece el color del texto
    ctx.textAlign = 'right';            // Alinea el texto a la derecha
    ctx.textBaseline = 'top';           // Alinea verticalmente a la parte superior
    ctx.block = 'center';
    ctx.fillText(text, canvas.width - 300, 50); // Dibuja el texto (10px de margen)
}
...
let enemySpawnRate = 2000; // Frecuencia inicial de generación de enemigos (2 segundos)
let lastEnemySpawn = 0; // Última vez que se generó un enemigo
